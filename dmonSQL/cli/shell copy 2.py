    ...
    def complete(self, text, state):
        ...
        if state == 0:
            ...
            if text:
                ...
                if self.db_system.current_database:
                    tables = self.db_system.current_database.list_tables()
                    self.matches.extend([
                ...
                databases = self.db_system.list_databases()
                self.matches.extend([
...
╚══════════════════════════════════════════════════════════════════════╝
    ...
    def run(self):
        ...
        while self.running:
            try:
                # Afficher le prompt
                prompt = f"dmonSQL[({self.db.current_db or 'None'})]> "
